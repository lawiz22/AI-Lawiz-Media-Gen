...
interface ImageGeneratorHeaderProps {
    ...
    restoreOptions: (options: GenerationOptions) => void;
...
export const ImageGeneratorHeader: React.FC<ImageGeneratorHeaderProps> = ({
    ...
    const presets = useSelector((state: RootState) => state.library.items.filter(item => item.mediaType === 'preset'));
    ...
    const filteredPresets = useMemo(() => {
        return presets.filter(preset => !!preset.options);
    }, [presets]);
    ...
    const selectedPresetName = useMemo(() => {
        if (!selectedPresetId) return '';
        const preset = presets.find(candidate => String(candidate.id) === selectedPresetId);
