...
export const ImageGrid: React.FC<ImageGridProps> = ({ images, onZoom }) => {
  ...
  const handleDownload = (imgSrc: string, index: number) => {
    ...
    link.href = imgSrc;
    link.download = `portrait_${index + 1}.png`;
  ...
  return (
    <div className="w-full h-full overflow-y-auto">
        ...
        {images.map((imgSrc, index) => (
            <div key={index} className="bg-bg-primary rounded-lg overflow-hidden shadow-md group relative">
              <img 
                  src={imgSrc} 
              ...
              <div className="absolute inset-0 bg-black/70 flex items-center justify-center gap-4 opacity-0 group-hover:opacity-100 transition-opacity duration-300">
                <button 
                  ...
                  className="p-3 bg-bg-tertiary/50 rounded-full text-white hover:bg-accent/80 transition-all transform hover:scale-110"
                  title="Zoom"
                >
